...
let db = window.openDatabase("FGW2", "1.0", "FGW2", 20000);
...
function confirmTrip(e) {
  ...
  $("#page-create #frm-confirm #other").text(
    $("#page-create #frm-register #other").val()
  );
  $("#page-create #frm-confirm #totalday").text(
    $("#page-create #frm-register #totalday").val()
  );
...
function registerTrip(e) {
  ...
  let other = $("#page-create #frm-register #other").val();
  let totalday = $("#page-create #frm-register #totalday").val();

  db.transaction(function (tx) {
    let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Date, Other, TotalDay) VALUES (?, ?, ?, ?, julianday('${date}'), ?, ?)`;
    tx.executeSql(
      query,
      [name, destination, description, risk, other, totalday],
      transactionSuccess,
      transactionError
    );
  ...
  db.transaction(function (tx) {
    ...
    function transactionSuccess(tx, result) {
      if (result.rows[0] != null) {
        ...
        $("#page-detail #detail #other").text(result.rows[0].Other);
        $("#page-detail #detail #totalday").text(result.rows[0].TotalDay);
  ...
  db.transaction(function (tx) {
    ...
    function transactionSuccess(tx, result) {
      if (result.rows[0] != null) {
        ...
        $(`#page-detail #frm-update #other`).val(result.rows[0].Other);
        $(`#page-detail #frm-update #totalday`).val(result.rows[0].TotalDay);
...
function updateTrip(e) {
  ...
  let other = $("#page-detail #frm-update #other").val();
  let totalday = $("#page-detail #frm-update #totalday").val();

  db.transaction(function (tx) {
    let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Date = julianday('${date}'),Other = ?,Totalday = ? WHERE Id = ?`;

    tx.executeSql(
      query,
      [name, destination, description, risk, other, totalday, id],
      transactionSuccess,
      transactionError
    );
...
function search(e) {
  ...
  let other = $("#page-list #frm-search #other").val();
  let totalday = $("#page-list #frm-search #totalday").val();
  ...
  db.transaction(function (tx) {
    ...
    query += other ? ` Other LIKE "%${other}%"   AND` : "";
    query += totalday ? ` TotalDay LIKE "%${totalday}%"   AND` : "";
